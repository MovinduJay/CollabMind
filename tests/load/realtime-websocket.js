...
const acknowledgementLatency = new Trend("message_acknowledgement_latency", true);
...
export default function () {
  ...
  if (!token || !conversationId) throw new Error("AUTH_TOKEN and CONVERSATION_ID are required");

  const started = Date.now();
  const response = ws.connect(`${gateway}?token=${encodeURIComponent(token)}`, {}, socket => {
    socket.on("open", () => {
      socket.send(JSON.stringify({ type: "SUBSCRIBE", payload: { conversationId } }));
      socket.send(JSON.stringify({
        type: "SEND_MESSAGE",
        payload: { conversationId, content: `k6 message ${__VU}-${__ITER}` }
      }));
    });
