...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(
    `http://localhost:8081/api/conversations/${roomId}`,
    async (route) => {
...
test("menus, participant drawer, and emoji picker toggle cleanly", async ({
  page,
}) => {
  const conversationMenu = page.getByRole("button", {
  ...
  await expect(
    page.getByRole("complementary", { name: "Room participants" }),
  ).toBeVisible();
  await expect(page.getByText("Maya (you)")).toBeVisible();
  await page
    .getByRole("complementary", { name: "Room participants" })
  ...
  await expect(
    page.getByPlaceholder("Enter the room to start chatting"),
  ).toHaveValue("😀");
...
test("room title editing and AI mention selection work", async ({ page }) => {
  ...
  await expect(page.getByTitle("Edit room title")).toContainText(
    "Launch planning",
  );
  ...
  await composer.fill("@");
  await expect(
    page.getByRole("button", { name: /@ai Ask the AI assistant/ }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: /@kapruka Search live Kapruka products/ })
    .click();
